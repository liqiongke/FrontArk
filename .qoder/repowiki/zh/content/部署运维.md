...
- 框架库：@jl/framework（UMD/ES 双格式输出，外部化 react/react-dom/antd）
...
Turbo->>Lib : 触发 @jl/framework 构建(依赖图)
Lib-->>FS : 生成 framework.*.js / .d.ts
...
- 框架库外部化
  - react、react-dom、antd 作为 peerDependencies 外部化，避免重复打包

```mermaid
graph LR
App["apps/demo"] --> FW["@jl/framework"]
App --> |Vite/TS/ESLint| Build["构建工具链"]
FW --> |peerDeps| Runtime["React/ReactDOM/AntD"]
...
## 结论
本项目基于 Vite + Turborepo + pnpm workspace 的现代前端工程体系，具备清晰的构建与部署边界。通过路由级代码分割、严格的缓存策略与完善的监控手段，可在生产环境获得良好的性能与稳定性。建议结合容器化与云服务最佳实践，完善 CI/CD、备份恢复与灾难恢复流程，持续提升交付效率与系统韧性。
